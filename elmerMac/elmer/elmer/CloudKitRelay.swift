...
extension CKRecord {
    ...
    func toAIRequest() -> AIRequest? {
        // Debug record parsing
        print("🔍 Debug: Parsing CKRecord with type: \(recordType)")
        print("🔍 Debug: Expected type: \(CloudKitRelayConfig.requestRecordType)")
        print("🔍 Debug: Record fields: \(self.allKeys())")
        
        guard recordType == CloudKitRelayConfig.requestRecordType else {
            print("❌ Record type mismatch: got '\(recordType)', expected '\(CloudKitRelayConfig.requestRecordType)'")
            return nil
        }
        
        guard let serviceID = self["serviceID"] as? String else {
            print("❌ Missing or invalid serviceID field")
            return nil
        }
        
        guard let serviceName = self["serviceName"] as? String else {
            print("❌ Missing or invalid serviceName field")
            return nil
        }
        
        guard let endpoint = self["endpoint"] as? String else {
            print("❌ Missing or invalid endpoint field")
            return nil
        }
        
        guard let method = self["method"] as? String else {
            print("❌ Missing or invalid method field")
            return nil
        }
        
        guard let timestamp = self["timestamp"] as? Date else {
            print("❌ Missing or invalid timestamp field")
            return nil
        }
        
        guard let deviceID = self["deviceID"] as? String else {
            print("❌ Missing or invalid deviceID field")
            return nil
        }
        
        guard let statusString = self["status"] as? String else {
            print("❌ Missing or invalid status field")
            return nil
        }
        
        guard let status = RequestStatus(rawValue: statusString) else {
    ...
    func toAIResponse() -> AIResponse? {
        guard recordType == CloudKitRelayConfig.responseRecordType,
              let requestID = self["requestID"] as? String,
              let statusCode = self["statusCode"] as? Int,
              let timestamp = self["timestamp"] as? Date,
              let processingTime = self["processingTime"] as? Double else {
            return nil
        }
...
class DeviceIdentityManager {
    static func getOrCreateDeviceID() -> String {
        ...
        let newID = "mac-" + UUID().uuidString.lowercased().prefix(8)
